...
!(async function run() {
  ...
  //   //@ts-ignore
  //   const { figi } = await api.searchOne({ ticker: "SBMX" }); // Поиск эмитента
  ...
  //   const glass = await api.orderbookGet({ figi, depth: 2 }); // получаем стакан по AAPL
...
app.get("/", (req, res) => {
  res
    .status(200)
    .json({ status: 6, message: "getErrorMessage getErrorMessage 558" });
});
